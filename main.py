...
def run_tip_adapter_F(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights, clip_model, train_loader_F):
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        # NOTE: changed
        for i, (images, target, _) in enumerate(tqdm(train_loader_F)):
...
def get_syn_dataset(cfg, images, target, lab2cname, num_classes, classnames, inv_cmap, template):
    '''
    Generate syn images based on class labels and captions
    '''
    ...
    for i in range(len(dataset_classname)): 
        #breakpoint()       
        # append the sampled images
        images_per_class = [ images[torch.where(target.cpu()==i)] ]
    ...
    #breakpoint()
    # get the model for image, text features 
    for i in range(len(dataset_classname)):
        caption_per_class = []    
        for j in range(num_shots):
            inputs = processor(sampled_images[i][0][j], return_tensors="pt").to("cuda", torch.float32)
            out = model.generate(**inputs)
            # get the caption
            caption = processor.decode(out[0], skip_special_tokens=True)
            
            # append
            caption_per_class.append(caption)
            
        caption_all_class.append(caption_per_class)

    #####################################################################

    #breakpoint()
    ####### modified captions using domain and class names ##############

    modified_caption_all_class = []
    for i in range(len(dataset_classname)):
        modified_caption_per_class = []
        for j in range(num_shots):
            # modified caption is DOMAIN + CLASS LABEL + CAPTION
            modified_caption = template[0].replace('{}', classnames[i]).strip('.') + ' and ' + caption_all_class[i][j]
            #modified_caption = 'a satellite image of ' + classnames[i] + ' and ' + caption_all_class[i][j]
            modified_caption_per_class.append(modified_caption)
        modified_caption_all_class.append(modified_caption_per_class)
    ...
    if not os.path.exists(dir_path):
        ...
        for i in range(len(dataset_classname)):
            ...
            # generate images for 5 images
            for j in range(num_shots):
                # k could vary
                for k in range(min(num_shots,5)):
                    gen_image = pipe(prompt=modified_caption_all_class[i][k], \
                        image=transforms.ToPILImage()(sampled_images[i][0][j]).convert("RGB").resize((512,512)), \
                            strength=0.75, guidance_scale=7.5).images[0]
                    gen_image.save(os.path.join(dir_path, dataset_classname[i], str(min(num_shots,5)*j+k)+'.png'))
                    #plt.imsave(os.path.join(dir_path, all_classnames[i], str(j)+'.png'),gen_image)
        
        #breakpoint()

    #breakpoint()
    ## generate json file
    generate_json(cfg, dataset=dataset, syn_data_folder=dir_path, dataset_folder=dataset_folder, dataset_classname=dataset_classname)
...
def get_caption(cfg, train_loader_F, template):
    ...
    # generate syn images 
    for ii, (images, target, idx) in enumerate(tqdm(train_loader_F)):
        images, target = images.cuda(), target.cuda()
...
def run_tip_adapter_F_syn(cfg, args, cache_keys, cache_values, val_features, val_labels, test_features,\
     test_labels, clip_weights, clip_model, train_loader_F, lab2cname, num_classes, classnames, \
        train_tranform, inv_cmap, template):
    ...
    get_syn_images(cfg, train_loader_F, template, captions_by_class, classnames, inv_cmap)

    # added loader for syn images
    # check the build dataset function
    syn_dataset = build_dataset(cfg['dataset'], cfg['root_path'], cfg['shots'], is_syn=True)
    ...
    syn_cache_keys, syn_cache_values = build_cache_model(cfg, clip_model, syn_loader_cache)
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (real_images, real_target, _) in enumerate(tqdm(train_loader_F)):
            ...
            #breakpoint()
            syn_img, syn_target, _ = next(iterator) #20000xD, len(20000)
...
def run_tip_adapter_F_syn_all(cfg, cache_keys, cache_values, val_features, val_labels, test_features,\
     test_labels, clip_weights, clip_model, train_loader_F, lab2cname, num_classes, classnames, \
        train_tranform, inv_cmap, template):
    ...
    syn_dataset = build_dataset(cfg['dataset'], cfg['root_path'], cfg['shots'], is_syn=True)
    # # check the syn_dataloader 
    syn_loader = build_data_loader(data_source=syn_dataset.train_u, batch_size=256, tfm=train_tranform, is_train=True, shuffle=True)
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (real_images, real_target, _) in enumerate(tqdm(train_loader_F)):
            ...
            for j, (syn_img, syn_target, _) in enumerate(tqdm(syn_loader)):

                print('no of iterate', j)
...
def main():
    ...
    dataset = build_dataset(cfg['dataset'], cfg['root_path'], cfg['shots'], is_syn=False)
    ...
    train_loader_cache = build_data_loader(data_source=dataset.train_x, batch_size=cfg['train_batchsize'], tfm=train_tranform, is_train=True, shuffle=False)
    train_loader_F = build_data_loader(data_source=dataset.train_x, batch_size=cfg['train_batchsize'], tfm=train_tranform, is_train=True, shuffle=True)
    ...
    #breakpoint()
    if cfg['dataset'] == 'eurosat':
        dataset.inv_cmap = dataset.inv_cmap
    else:
        dataset.inv_cmap = None
    ...
    elif args.mode=='baseline_naive_data_aug':
        # ------------------------------------------ Tip-Adapter-F + naive data aug ------------------------------------------
        run_tip_adapter_F(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights, clip_model, train_loader_F)
    else:
        # ------------------------------------------ Tip-Adapter-F Syn ------------------------------------------    
        run_tip_adapter_F_syn(cfg, args, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights, clip_model, train_loader_F, \
            dataset._lab2cname, dataset._num_classes, dataset._classnames, train_tranform, dataset.inv_cmap, dataset.template)
